...
import com.joopie.ffconverter.converters.pet.PetJSONMapper;
import com.joopie.ffconverter.converters.pet.models.*;
...
public class PetConverter  implements IConverter {
    private static final PetJSONMapper petJSONMapper = new PetJSONMapper();
    ...
    private PetJSON convertXML2JSON(HabboAssetSWF habboAssetSWF) throws Exception {
        ...
        return petJSONMapper.mapXML(habboAssetSWF, assetsXML, indexXML, logicXML, visualizationXML);
    }

    @Override
    public void fromHabboAsset(HabboAssetSWF habboAssetSWF, File outputFolder, String type) throws Exception {
        PetJSON petJSON = this.convertXML2JSON(habboAssetSWF);
        ObjectMapper objectMapper = new ObjectMapper();
        ...
        petJSON.setSpritesheet(jsonNode);
        SpritesheetConverter.spritesheetJSON = new SpritesheetJSON();
        //petJSON.setSpritesheet(habboAssetSWF.getDocumentClass() + "_spritesheet.json");

        this.getObjectMapper().writeValue(new File(outputFolder + "/" + habboAssetSWF.getDocumentClass() + ".json"), petJSON);
    }
}
